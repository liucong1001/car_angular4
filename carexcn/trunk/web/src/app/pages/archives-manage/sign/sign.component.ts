...
export class SignComponent implements OnInit {
  ...
  btn_plateNumber = false;
  ngOnInit() {
    this.model.cloudUser = '0001';
  }
  show() {
   this.btn_plateNumber = !this.btn_plateNumber;
  }
  /**
   * 车管所流水号--签收
   * @param event
  ...
  arcNoSearch(event: any) {
    ...
    if (event.keyCode == 13 && String(this.model.archivesNo).length == 16) {
      this.sendService.getPlateNumber(this.model.archivesNo).then(res => {
        this.model.plateNumber = res.preVehicle.preVehicle.plateNumber;
        this.sign(this.model.plateNumber);
      });
    }
  }
  ...
  plateNumberSearch(event: any) {
    if (event.keyCode === 13) {
        if (String(this.plateNumber).length === 7 || String(this.plateNumber).length === 8){
          this.model.plateNumber = this.plateNumber;
          this.sign( this.model.plateNumber);
        }else {
          this.message.error('请输入正确长度的车牌号！', '');
        }
    }
  }
