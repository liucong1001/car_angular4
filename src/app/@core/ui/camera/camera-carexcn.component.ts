...
import {FileSystemService, CameraCarexcnFileDescrption} from './../../data/system/file-system.service';
@Component({
...
// export class CameraCarexcnComponent {
   ...
   * @param {string} file
   */
  afterGetFileName(file: CameraCarexcnFileDescrption) {
    /**
     * 显示当前新图片
     */
    this.source = this.file.getFileUrlByTmp(file.filePath);
  ...
  paizhao() {
    ...
    this.webcam.snapshot(false, 'a', 'b').then((res) => {
      this.afterGetFileName({'filePath': res.file[0]} as CameraCarexcnFileDescrption);
    });
  }
  /**
   * 本地照片上传成功时显示
  ...
  onUploadComplete(source) {
    this.afterGetFileName({'filePath': source} as CameraCarexcnFileDescrption);
  }
}
